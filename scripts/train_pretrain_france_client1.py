...
# ── Module-level shared memory for gRPC comm stats (set in main() before DataLoader) ──
_comm_bytes_shm = None   # mp.Value('Q') – set by main()
_comm_calls_shm = None
...
def global_worker_init_fn(worker_id,dataset_obj=None):
    ...
    # ── Patch gRPC in this worker BEFORE the .pyd creates any stub ──
    if _comm_bytes_shm is not None and _comm_calls_shm is not None:
        import grpc as _grpc
        import grpc._channel as _grpc_ch
        import functools as _ft
        bv = _comm_bytes_shm
        cv = _comm_calls_shm
        _orig_unary = _grpc_ch.Channel.unary_unary

        _traced_count = [0]  # mutable counter for debug

        def _patched(channel_self, method, **kw):
            mc = _orig_unary(channel_self, method, **kw)
            _orig_call = mc.__call__

            @_ft.wraps(_orig_call)
            def _traced(request, *a, **kw2):
                try:
                    req_bytes = request.ByteSize()
                except Exception:
                    req_bytes = 0
                result = _orig_call(request, *a, **kw2)
                bv.value += req_bytes
                cv.value += 1
                _traced_count[0] += 1
                return result
        ...
        _grpc_ch.Channel.unary_unary = _patched
        _grpc.Channel.unary_unary = _patched
        print(f"[Worker {worker_id}] ✅ gRPC unary_unary patched (shared-memory mode)", flush=True)
...
class GPUMonitor:
    ...
    When *bytes_shm* / *calls_shm* (``multiprocessing.Value``) are provided,
    ``stats()`` also merges counts accumulated inside DataLoader worker
    subprocesses.
    """

    def __init__(self, bytes_shm=None, calls_shm=None):
        self.comm_bytes = 0
        self.comm_calls = 0
        ...
        self.active = False
        self._bytes_shm = bytes_shm    # mp.Value('Q') – written by worker
        self._calls_shm = calls_shm
        self._grpc_patched = False
        self._model_wrapped = False
        # Accurate measurement of network traffic (read / proc/net/dev, summary of all web portals including lo)
        self._net_rx_snapshot = 0
        self._net_tx_snapshot = 0

    def _read_net_bytes(self):
        """Return (rx_bytes, tx_bytes) summed across ALL interfaces including lo."""
        total_rx = 0
        total_tx = 0
        try:
            for line in open("/proc/net/dev", "r").readlines()[2:]:
                ...
                if len(fields) >= 9:
                    total_rx += int(fields[0])   # rx bytes
                    total_tx += int(fields[8])   # tx bytes
        except Exception:
            pass
        return total_rx, total_tx

    def net_snapshot(self):
        """Take a snapshot of current network counters (call at start of interval)."""
        rx, tx = self._read_net_bytes()
        self._net_rx_snapshot = rx
        self._net_tx_snapshot = tx

    def net_delta(self):
        """Return (delta_rx, delta_tx) since last snapshot, then update snapshot."""
        rx, tx = self._read_net_bytes()
        drx = max(0, rx - self._net_rx_snapshot)
        dtx = max(0, tx - self._net_tx_snapshot)
        self._net_rx_snapshot = rx
        self._net_tx_snapshot = tx
        return drx, dtx
    ...
    def _setup_grpc_trace(self, orig_fn):
        ...
        mon = self
        def _patched_unary_unary(channel_self, method, **kwargs):
            multi_callable = orig_fn(channel_self, method, **kwargs)
    ...
    def start(self):
        ...
        try:
            ...
            # Patch the ABSTRACT base class (for any custom subclasses)
            grpc.Channel.unary_unary = self._setup_grpc_trace(self.orig_grpc_unary_unary[0])

            # Patch the CONCRETE implementation (this is what grpc.insecure_channel actually uses)
            # This is the critical fix: grpc._channel.Channel overrides the base unary_unary,
            # so patching only the base class had no effect.
            grpc._channel.Channel.unary_unary = self._setup_grpc_trace(self.orig_grpc_unary_unary[1])
            self._grpc_patched = True
            print("[GPUMonitor] ✅ gRPC unary_unary patched successfully (main process)")
        except Exception as e:
            print(f"[GPUMonitor] ⚠️ gRPC patch failed: {e}")
    ...
    def stats(self):
        ...
        total_calls = self.comm_calls
        # Merge counts from DataLoader worker subprocess (if any)
        if self._calls_shm is not None:
            total_calls += self._calls_shm.value
        # Real gRPC bytes: delta from /proc/net/dev (all interfaces incl. lo)
        delta_rx, delta_tx = self.net_delta()
    ...
    def wrap_model_forward(self, model, hidden_size=None):
        """Wrap model.__call__ — only counts calls for the call counter.
        Real gRPC bytes come from /proc/net/dev (net_delta).
        Real gRPC time is estimated in _log_training_step from GPU util."""
        mon = self
        _orig_call = model.__class__.__call__

        @functools.wraps(_orig_call)
        def _traced_forward(model_self, *args, **kwargs):
            result = _orig_call(model_self, *args, **kwargs)
            mon.comm_calls += 1  # just count, no timing
            return result

        model.__class__.__call__ = _traced_forward
        self._model_wrapped = True
        print(f"[GPUMonitor] ✅ Model forward wrapped (net bytes from /proc/net/dev)")
...
def parse_args():
    ...
    # data
    p.add_argument("--train_folder", type=str,default="../MandelbrotV1/10GB", help="Folder containing .txt files (recursively)")
    ...
    p.add_argument("--pad_mode", type=bool, default=False, help="Whether to use fill mode") 
    ...
    p.add_argument("--layer_idx", type=int, default=-1, help="Index of the layer to use for tokenizer")

    # optimization (client-side)
    p.add_argument("--per_device_train_batch_size", type=int, default=4)  # Increase batch_size to make full use of GPU memory
    p.add_argument("--prefetch_size", type=int, default=8, help="Number of batches to prefetch in the background")
    p.add_argument("--max_steps", type=int, default=-1)
    p.add_argument("--max_epochs", type=int, default=2, help="Maximum number of training epochs")
    p.add_argument("--min_loss", type=float, default=-1, help="Minimum loss threshold to stop training")
    ...
    # runtime
    p.add_argument("--output_dir", type=str, default="../MandelbrotV1/outputs_incremental_pretraining9", help="Output directory for checkpoints")
    ...
    p.add_argument("--enable_gpu_monitor", type=bool, default=True, help="Enable GPU UTILZATION surveillance (pynvml required)")
    p.add_argument("--enable_comm_monitor", type=bool, default=False, help="Enable traffic monitoring (gRPC / proc/net/dev statistics)")
    ...
    p.add_argument("--mem_threshold", type=float, default=0.85, help="System threshold (default 0.85)")
    p.add_argument("--vram_threshold", type=float, default=0.75, help="GPU memory threshold (default 0.85)")
    ...
    p.add_argument("--distribution_Type", type=str, default="client", choices=["server", "client","middle_Server","local","incremental_pretrain_full","incremental_pretrain"], help="Distributed training type (is_Distribution=True Effective): server, middle_Server, clit Local training type (is_Distribution=False Effective): local, incremental_pretain_full, incremental_pretrain")
    p.add_argument("--worker_id", type=str, default="server_1", help="Worker id for distributed training")
    p.add_argument("--model_name_or_path", type=str, default="../MandelbrotV1/configs/client", help="Path to pre-trained model or model identifier from huggingface.co/models")
    ...
    p.add_argument("--dataloader_num_workers", type=int, default=1, help="Number of subprocesses for data loading")
...
def main():
    ...
    # ===== ★ Communication Monitor Must Be Initialized Before model Loading ★ =====
    # GRPC stub calls Channel.unary_unary() and caches MultiCallable,
    # If Mon.start() is not affected by the created stub until the model is loaded.
    if getattr(args, "enable_comm_monitor", True):
        mon = GPUMonitor()
        mon.start()
        mon.net_snapshot()  # Initialization of network traffic baseline snapshots
        print("✅ Communication monitor activated (gRPC patch effective before model is created)")
    else:
        mon = None
        print("Communications monitor disabled")
...
# ==================================================================
    ...
    # ── ★ Shared memory must be set before DataLoader is created -
    # DataLoader is created with a sub-process fork worker, only then _comm_bytes_shm is None,
    # GRPP catch from worker
    comm_bytes_shm = mp.Value('Q', 0)   # unsigned long long: bytes
    comm_calls_shm = mp.Value('Q', 0)   # unsigned long long: calls
    global _comm_bytes_shm, _comm_calls_shm
    _comm_bytes_shm = comm_bytes_shm
    _comm_calls_shm = comm_calls_shm

    if mon is not None:
        # Update shared memory references for monitors (combining traffic from DataLoader worker subprocess)
        mon._bytes_shm = comm_bytes_shm
        mon._calls_shm = comm_calls_shm
        print("✅ Shared memory counter linked to the communications monitor")

        # If the grPC patch is not in effect or is in the clit mode (gRPC in C extension, Python layer cannot be intercepted),
        # Back to Packaging model timed
        _hidden_size = int(getattr(config, "hidden_size", 0) or 0)
        if not mon._grpc_patched:
            mon.wrap_model_forward(model, hidden_size=_hidden_size)
            print("⚠️ gRPC patch not effective, returned to model forecast mode")
        elif args.is_Distribution and args.distribution_Type == "client":
            mon.wrap_model_forward(model, hidden_size=_hidden_size)
            print("⚠️ Client mode: gRPC Python catch cannot intercept C-level calls back to model timer")
    ...
    last_log_time = [start_time]  # mutable for updating in _log_training_step
    epoch = resume_epoch
    ...
    def _log_training_step(global_step, epoch, synced_running_loss, steps_since_last_log, current_non_zero_count_ratio, current_indep_loss_ratio, scheduler, start_time, tb_writer, args, avg_perf, dataset=None, loss_list=None):
        ...
        # Communication spending statistics: bytes from /proc/net/dev (OS-grade real grPC flows),
        # Time estimate = time-consuming zone x (1 - GPU UTILZATION/100)
        if mon is not None:
            s = mon.stats()
            if s["calls"] > 0 and s["mb"] > 0:
                gpu_util = avg_perf.get('avg_gpu_util', 50) if avg_perf else 50
                now = time.time()
                interval_s = max(0.001, now - last_log_time[0])
                last_log_time[0] = now
                est_comm_s = interval_s * max(0, 1.0 - gpu_util / 100.0)
                if s["mb"] >= 1.0:
                    comm_str = f" | Comm:{s['calls']}calls {s['mb']:.1f}MB ~{est_comm_s:.0f}s"
                else:
                    comm_str = f" | Comm:{s['calls']}calls {s['mb']*1024:.0f}KB ~{est_comm_s:.0f}s"
            elif s["calls"] > 0:
                comm_str = f" | Comm:{s['calls']}calls"
            else:
                comm_str = ""
        else:
            comm_str = ""
            # Diagnostic: Output state at first logging
            if mon is not None and global_step <= args.logging_steps:
                grpc_status = "patched" if getattr(mon, '_grpc_patched', False) else "NOT patched"
                fwd_status = "wrapped" if getattr(mon, '_model_wrapped', False) else "not wrapped"
                print(f"[CommDiag] gRPC={grpc_status}, model_forward={fwd_status}, "
                      f"is_Distribution={args.is_Distribution}, dist_Type={args.distribution_Type}",
                      flush=True)
        if comm_str:
            print(f"{comm_str}")
    ...
    for epoch in range(resume_epoch, effective_num_train_epochs):
        for batch in dataloader:
            ...
            if  should_log:
                ...
                # Reset communication monitor counter (including shared memory)
                if mon is not None:
                    mon.comm_calls = 0; mon.comm_bytes = 0; mon.comm_cpu_us = 0.0
                    mon.net_snapshot()  # Reset network traffic snapshots and count next compartment
                comm_bytes_shm.value = 0; comm_calls_shm.value = 0
                perf_records.clear()
            ...
            global_step += 1
            # Reset network snapshots after first micro-batch to exclude all delayed initialization traffic
            if mon is not None and global_step == 1:
                mon.net_snapshot()
